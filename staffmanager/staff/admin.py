...
"""
# Для добавления полей модели сотрудника-пользователя (Employee) на страницу  пользователя (User) 
# Определяем встроенный дескриптор администратора для модели Employee
# который работает как синглтон
class EmployeeInline(admin.StackedInline):
    model = Employee
    can_delete = False
    verbose_name_plural = "сотрудники"

# Определение нового администратора пользователей
class UserAdmin(BaseUserAdmin):
    inlines = [EmployeeInline]    

# Повторная регистрация UserAdmin
